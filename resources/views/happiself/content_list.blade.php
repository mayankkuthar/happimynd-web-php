                  ...
                  <div class="x_content">
                    <table id="datatable" class="table table-striped table-bordered" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>{{ ++$i }}</td>
                          <td>{{$row->courseName->course_name ?? ''}}</td>
                          <td>{{$row->subCourseName->sub_course_name ?? ''}}</td>
                          <td>{{$row->content_type ?? ''}}</td>
                          <td>{{$row->content ?? ''}}</td>
                          <td style="text-align: center;">
                            <a href="{{url('admin/delete-happiself-content').'/'.$row->id}}" class="btn btn-primary">Delete</a>
                          </td>
                        </tr>
                        @endforeach
