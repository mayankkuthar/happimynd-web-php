            ...
            <div class="col-md-12 col-sm-12 ">
              <div class="x_panel">
                ...
                <div class="x_content">
                    ...
                    <form class="user-form" action="{{ route('admin.addAdminUser') }}" method="post" novalidate>
                        @csrf
                        <div class="field item form-group @error('first_name') bad @enderror">
                            ...
                            <div class="col-md-6 col-sm-6">
                                <input class="form-control" name="first_name" placeholder="John" required="required" value="{{ old('first_name') }}" />
                            </div>
                            @error('first_name')
                        ...
                        <div class="field item form-group @error('last_name') bad @enderror">
                            ...
                            <div class="col-md-6 col-sm-6">
                                <input class="form-control" name="last_name" type="text" placeholder="Doe" value="{{ old('last_name') }}"/>
                            </div>
                            @error('last_name')
                        ...
                        <div class="field item form-group @error('mobile') bad @enderror">
                            ...
                            <div class="col-md-6 col-sm-6">
                                <input class="form-control" type="number" class='number' name="mobile" required='required' value="{{ old('mobile') }}">
                            </div>
                            @error('mobile')
                            ...
                            <div class="col-md-6 col-sm-6">
                                @foreach($roles as $role)   
                                    @if($role->name != 'content-writer' && $role->name != 'psychologist')
                                        <input type="checkbox" name="roles[]" id="role{{ $role->id }}" value="{{ $role->name }}" required class="flat" /> {{ $role->name }}
                                        <br>
                                    @endif
                                @endforeach
                            </div>
