                ...
                <form id="expert-level-form">
                  <div class="field item form-group @error('first_name') bad @enderror">
                    ...
                    <div class="col-md-6 col-sm-6">
                      <input class="form-control" name="name" placeholder="Clinical Psychologist" required="required" value="{{ old('name') }}" />
                    </div>
                    @error('expert_level')
                  ...
                  <div class="field item form-group @error('duration-{{ $duration->id }}-cost-price') bad @enderror">
                    ...
                    <div class="col-md-6 col-sm-6">
                      <input class="form-control" name="plans[{{ $duration->id }}][cost-price]" type="text" placeholder="enter amount" value="{{ old('duration-'.$duration->id) }}"/>
                    </div>
                  ...
                  <div class="field item form-group @error('duration-{{ $duration->id }}-cost-price-discount') bad @enderror">
                    ...
                    <div class="col-md-6 col-sm-6">
                      <input class="form-control" name="plans[{{ $duration->id }}][cost-price-discount]" type="text" placeholder="enter amount" value="{{ old('duration-'.$duration->id.'-cost-price-discount') }}"/>
                    </div>
                    @error('duration-{{ $duration->id }}')
                  ...
                  <div class="field item form-group @error('duration-{{ $duration->id }}-selling-price') bad @enderror">
                    ...
                    <div class="col-md-6 col-sm-6">
                      <input class="form-control" name="plans[{{ $duration->id }}][selling-price]" type="text" placeholder="enter amount" value="{{ old('duration-'.$duration->id.'-selling-price') }}"/>
                    </div>
                    @error('duration-{{ $duration->id }}')
    ...
    <script type="text/javascript">
      $('#expert-level-form').on('submit', function(e){
        ...
        $.ajax({
          method: "POST",
          data: formData,
          url: "{{ route('admin.addExpertLevel.post') }}",
          dataType: "JSON",
