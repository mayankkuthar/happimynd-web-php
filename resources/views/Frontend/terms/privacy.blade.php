...
@section('title', 'Happimynd | Privacy Policy')
    ...
    <nav class="navbar terms__navbar">
      <div class="container">
        <div class="landingpage__logo">
          <a href="{{ url('/') }}" title="Click on logo to go to home"><img src="{{ asset('assets/Frontend/images/happimynd_logo.png') }}" />| Privacy Policy</a>
        </div>
        <div class="download__terms">
          <a href="https://happimynd.s3.ap-south-1.amazonaws.com/assets/static+assets/HappiMynd_Privacy+Policy_download.pdf" target="_blank">
          ...
          <div class="terms__content__options">
            <ul>
              @foreach($dataContent->content as $content)
                <li><a class="" href="{{ route('privacy') }}#{{ $content->title }}">{{ $loop->iteration }}. {{ $content->title }}</a></li>
              @endforeach
            </ul>
          </div>
        </div>
