...
class AssessmentServiceOld
{
    ...
    public function getQuestionsApp($assessmentId)
    {
        ...
        if ($this->questionsPerPage == null) {
            ...
            if (empty($answered_question_ids) && $this->checkIfProfileHasPersonalityQuestions()) {
                $batchCategoryIds = $this->batchCategory->pluck('id')->toArray();
                // $questions = $this->getPersonalityQuestionsApp();
                // if($questions == null){
                ...
                sort($answered_question_ids);
                // $personalityQuestions = $this->getRemainingPersonalityQuestionsApp();
    ...
    public function getPersonalityQuestionsApp($limit = true)
    {
        $batchCategoryIds = $this->batchCategory->pluck('id')->toArray();
        $personalityQuestions = Question::where('language' , 'hindi')->whereHas('category', function ($query) {
            $query->where('name', 'like', 'Personality');
        })
            ->whereHas('option')
            ->whereIn('batch_category_id', $batchCategoryIds)
    ...
    public function getRemainingPersonalityQuestionsApp()
    {
        $answeredQuestionIds = $this->getAnsweredQuestions()->pluck('id')->toArray();
        $batchCategoryIds = $this->batchCategory->pluck('id')->toArray();
        $remainingQuestions = Question::where('language' , 'hindi')->whereHas('category', function ($query) {
            $query->where('name', 'like', 'personality');
        })
            ->whereHas('option')
            ->whereIn('batch_category_id', $batchCategoryIds)
